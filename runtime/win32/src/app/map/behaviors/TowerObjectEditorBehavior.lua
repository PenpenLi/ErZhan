...
local TowerObjectEditorBehavior = class("TowerObjectEditorBehavior", BehaviorBase)

TowerObjectEditorBehavior.FIRE_CIRCLE_SELECTED_COLOR = {0, 0, 0, 255}
TowerObjectEditorBehavior.FIRE_CIRCLE_UNSELECTED_COLOR = {90, 0, 0, 180}

function TowerObjectEditorBehavior:ctor()
    TowerObjectEditorBehavior.super.ctor(self, "TowerObjectEditorBehavior", nil, 0)
end

function TowerObjectEditorBehavior:bind(object)
    ...
    local function createView(object, batch, marksLayer, debugLayer)
        ...
        -- change by Owen, 2015.5.12, 把utils.drawCircle改为display.newCircle
        -- 因为utils.drawCircle不会随着debugerLayer的scale进行缩放，缩小后仍保持原来的大小
        -- 也就是说不会随着父类进行缩放
        -- 给人一种进入了射程怎么还不攻击的错觉，实际上没有进入射程
    ...
    local function updateView(object)
        ...
        if object:hasBehavior("FireBehavior") then
            -- 塔升级了以后，debugLayer显示的射程范围也要更新
            local circle = object.fireRangeCircle_
            if circle.radius ~= object.fireRange_ then
                circle:setRadius(object.fireRange_)
            end

            circle:setPosition(x + object.radiusOffsetX_, y + object.radiusOffsetY_)
            if object.isSelected_ then
                circle:setLineColor(cc.c4fFromc4b(cc.c4b(unpack(TowerObjectEditorBehavior.FIRE_CIRCLE_SELECTED_COLOR))))
                -- object.fireRangeCircle_:setLineStippleEnabled(false)
            else
                circle:setLineColor(cc.c4fFromc4b(cc.c4b(unpack(TowerObjectEditorBehavior.FIRE_CIRCLE_UNSELECTED_COLOR))))
                -- object.fireRangeCircle_:setLineStippleEnabled(true)
            end
        end
...
function TowerObjectEditorBehavior:unbind(object)
...
return TowerObjectEditorBehavior
