...
function PathTool:ctor(toolbar, map)
    ...
    self.buttons = {
        {
            name          = "CreatePath",
            image         = "#MapEditor.CreatePathButton",
            imageSelected = "#MapEditor.CreatePathButtonSelected",
        },
        {
            name          = "SelectPathPoint",
            image         = "#MapEditor.SelectPathPointButton",
            imageSelected = "#MapEditor.SelectPathPointButtonSelected",
        },
        {
            name          = "AddPathPoint",
            image         = "#MapEditor.AddPathPointButton",
            imageSelected = "#MapEditor.AddPathPointButtonSelected",
            imageDisabled = "#MapEditor.AddPathPointButtonDisabled",
        },
        {
            name          = "RemovePathPoint",
            image         = "#MapEditor.RemovePathPointButton",
            imageSelected = "#MapEditor.RemovePathPointButtonSelected",
            imageDisabled = "#MapEditor.RemovePathPointButtonDisabled",
        },
        {
            name          = "RemovePath",
            image         = "#MapEditor.RemovePathAllPointButton",
            imageSelected = "#MapEditor.RemovePathAllPointButtonSelected",
            imageDisabled = "#MapEditor.RemovePathAllPointButtonDisabled",
        },
    }
end
...
function PathTool:setMoreButtonsEnabled(isEnabled)
    ...
    self.buttons[5].sprite:setButtonEnabled(isEnabled)
end
...
function PathTool:onTouch(event, x, y)
    ...
    elseif self.selectedButtonName_ == "RemovePath" then
        return self:onTouchRemovePath(event,x,y)
    end
end
...
function PathTool:onTouchSelectPathPoint(event, x, y)
    print("onTouchSelectPathPoint 1")
    if event == "began" then
        -- 检查是否选中了某条路径上的点
        print("onTouchSelectPathPoint 2")
        ...
        print("onTouchSelectPathPoint 3")

        return PathTool.TOUCH_IGNORED

    elseif event == "moved" then
        print("onTouchSelectPathPoint 4")
        self:movePoint(x, y)
        print("onTouchSelectPathPoint 5")
    else
        print("onTouchSelectPathPoint 6")
        self.currentPointIndex_ = nil
        if self.currentPointLabel_ then
            self.currentPointLabel_:setVisible(false)
        end
        print("onTouchSelectPathPoint 7")
    ...
    if event == "began" then
        ...
        if index then
            if self.currentPath_:getPointsCount() == 1 then
                -- 路径中只有一个点，直接删除整个路径
                self:onTouchRemovePath("began", x, y)
            else
                -- 删除选中的点
...
function PathTool:onTouchRemovePath(event, x, y)
    if event == "began" then
        -- 因为删完以后没有变工具，所以需要最外面加这么一个保护
        if self.currentPath_ then 
            local index = self.currentPath_:checkPointAtPoint(x, y, EditorConstants.CHECK_POINT_DIST)
            if index then

                -- 删除路径中所有的点
                self.currentPath_:removeAllPoint()
                self.map_:removeObject(self.currentPath_)
                self:setCurrentPath()
                -- TO-DO 如果删完路径以后，默认选中这个工具就会报错，先不改变选中的工具
                -- self.toolbar_:selectButton("PathTool", 2)
                return false
            end
        end
