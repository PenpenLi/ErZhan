...
-- 当点击工具栏中的按钮时，调用的是这个函数
function Toolbar:onButtonTap(selectedTool, selectedButton)
    for toolName, tool in pairs(self.tools_) do
        print("Toolbar:onButtonTap selectedTool = "..tostring(selectedTool)..
            " selectedButton = "..tostring(selectedButton))
        if tool ~= selectedTool then
            for i, buttonSprite in ipairs(tool.buttonsSprite) do
                buttonSprite:setButtonSelected(false)
                -- change by Owen, 2015.4.28, 取消所有选中的物体
                tool:unselected()
                -- change by Owen, 2015.5.14, 把地图设为可以拖动的
                self.map_:enableIgnoredMove(false)
            end
        end
