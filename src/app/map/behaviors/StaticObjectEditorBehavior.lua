...
function StaticObjectEditorBehavior:bind(object)
    ...
    local function createView(object, batch, marksLayer, debugLayer)
        -- print("StaticObjectEditorBehavior createView")
        ...
        if object:hasBehavior("FireBehavior") then
            ...
            object.fireRangeCircle_:setScaleY(0.8)
            debugLayer:addChild(object.fireRangeCircle_)

            -- change by Owen, 2015.5.10, 视野范围
            if object.fireVision_ then
                -- print(" if object.fireVision_ object.fireVision_ = "..tostring(object.fireVision_))
                object.fireVisionCircle_ = display.newCircle(object.fireVision_)
                -- object.fireVisionCircle_:setScaleY(0.8)
                -- object.fireVisionCircle_:setLineStipple(tonumber("1111000000000000", 2))
                -- object.fireVisionCircle_:setLineStippleEnabled(true)
                object.fireVisionCircle_:setScaleY(0.8)
                debugLayer:addChild(object.fireVisionCircle_)
            end
        end

        if object:hasBehavior("HeroBehavior") then
            object.baDaoRangeCircle_ = display.newCircle(object.patrolRange_)
            object.baDaoRangeCircle_:setScaleY(0.8)
            debugLayer:addChild(object.baDaoRangeCircle_)
        end
    ...
    local function removeView(object)
        ...
        if object.fireVisionCircle_ then
            object.fireVisionCircle_:removeSelf()
            object.fireVisionCircle_ = nil
        end
        if object.baDaoRangeCircle_ then
            object.baDaoRangeCircle_:removeSelf()
            object.baDaoRangeCircle_ = nil
        end
    ...
    local function updateView(object)
        -- print("StaticObjectEditorBehavior updateView")
        ...
        if object:hasBehavior("FireBehavior") then
            ...
            -- 视野范围
            if object.fireVisionCircle_ then
                local circle = object.fireVisionCircle_ 
                circle:setPosition(x + object.radiusOffsetX_, y + object.radiusOffsetY_)
            end
        end
        ...
        if object:hasBehavior("HeroBehavior") then
            if object.baDaoRangeCircle_ then
                local x, y = math.floor(object.patrolX_), math.floor(object.patrolY_)
                object.baDaoRangeCircle_:setPosition(x + object.radiusOffsetX_ , y + object.radiusOffsetY_)
                object.baDaoRangeCircle_:setLineColor(cc.c4fFromc4b(cc.c4b(unpack({100, 200, 0, 180}))))
            end
        end
