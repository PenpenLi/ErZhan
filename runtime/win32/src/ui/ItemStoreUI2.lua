...
function ItemStoreUI:ctor(self, finc, itemIndex)
    ...
    self.studioPage = import("ui.ItemStoreStudio").create()
...
function ItemStoreUI:getAllElemets()
    ...
    -- 当前拥有多少个钻石
    self.allHasDiamondLab = self.studioPage["allHasDiamond"]
    self.allHasDiamondLab:setFontName(GLOBAL_FONTNAME)
    self.allHasDiamondLab:enableOutline(cc.c4b(0,0,0,255),2)
    self.allHasDiamondLab:setString(GlobalData.getDiamondCount())

    -- 左侧列表中，3个道具icon
    self.itemIcons = {}
    -- 记录左侧3个icon是不是在播放缩放动画
    self.isIconInActions = {}
    for i = 1, 3 do
        self.itemIcons[i] = self.studioPage["item_"..tostring(i)]
        self.isIconInActions[i] = false
    end

    -- 3个item道具数量label
    self.itemCountLabs = {}
    for i = 1, 3 do
        self.itemCountLabs[i] = self.studioPage["itemCountLab"..tostring(i)]
        self.itemCountLabs[i]:setString(GlobalData.getSkillItemsCount(i))
        self.itemCountLabs[i]:setFontName(GLOBAL_FONTNAME)
        self.itemCountLabs[i]:enableOutline(cc.c4b(0, 0, 0, 255), 2)
    end

    -- 3个item道具按钮
    self.itemBtns = {}
    for i = 1, 3 do
        self.itemBtns[i] = self.studioPage["itemBtn_"..tostring(i)]

        self.itemBtns[i]:addClickEventListener(function (params )
            AudioMgr.playSound(AudioMgr.allUIEffect.Menu,false)
            self:onClickItemBtn(i)
        end)
    end

    -- item 技能描述label
    self.itemExplanLab = self.studioPage["itemExplanLab"]

    -- 右边的道具技能icon
    self.right_itemIcon = self.studioPage["right_itemIcon"]

    -- 道具技能的名字
    self.itemSkillName = self.studioPage["skillNameLab"]
    self.itemSkillName:setFontSize(30)
    self.itemSkillName:setFontName(GLOBAL_FONTNAME)
    self.itemSkillName:enableOutline(cc.c4b(72, 49, 30, 255), 2)

    -- 生成3个道具技能的描述
    self:dealSkillExplan()

    -- 购买item技能需要消耗的钻石数量
    self.needDiamondLabs = {}
    for i = 1, 3 do
        local skillConfig = RMBSkillProperties.get("710"..tostring(i).."01")
        self.needDiamondLabs[i] = self.studioPage["itemPriceLab"..tostring(i)]
        self.needDiamondLabs[i]:setFontName(GLOBAL_FONTNAME)
        self.needDiamondLabs[i]:setString(skillConfig.powerCost[2])
        self.needDiamondLabs[i]:enableOutline(cc.c4b(0, 0, 0, 255), 2)
    end

    -- 购买按钮
    local buyBtn  = self.studioPage["buyBtn"]
    buyBtn:addClickEventListener(function (params )
        AudioMgr.playSound(AudioMgr.allUIEffect.Menu,false)
        self:onClickBuy()
    end)

    -- 购买文字
    local buyLabel = self.studioPage["buyLab"]
    buyLabel:setFontName(GLOBAL_FONTNAME)
    buyLabel:enableOutline(cc.c4b(0, 0, 0, 255), 2)

    -- 默认显示第一个道具技能
    if self.itemIndex then
        self:onClickItemBtn(self.itemIndex)
    else
        self:onClickItemBtn(1)
    end
